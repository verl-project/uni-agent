#!/usr/bin/env bash
# ============================================================================
# RL training for Qwen3-Coder-30B-A3B-Instruct (SWE-Agent) WITH tricks.
#
# Same base as train_qwen3_coder_30b.sh (GRPO + GSPO loss, fully-async rollout),
# plus two add-ons on top of the bare baseline:
#
#   Trick 1) Decoupled PPO (algorithm.rollout_correction.bypass_mode=False)
#            + Rollout Correction (Rollout Importance Sampling).
#            Recomputes old_log_prob with the training engine (pinned to the
#            param version at the start of each sync window = stable proximal
#            anchor pi_old), then corrects the pi_old-vs-pi_rollout gap with IS
#            weights. ~= AReaL's Decoupled PPO under async stream + partial
#            rollout. Costs one extra old_log_prob forward per data batch.
#
#   Trick 2) 30B MoE Router Replay (R3): the rollout engine records its expert
#            routing and the trainer replays the exact same routes, removing
#            train/inference routing mismatch for the MoE layers.
#            R3 == rollout-side replay, so it pairs with
#            rollout.enable_rollout_routing_replay=True. (R2 and
#            enable_rollout_routing_replay are mutually exclusive.)
#
# Prereqs (already provided by fully_async_ppo_megatron_trainer.yaml + below):
#   - actor_rollout_ref.rollout.calculate_log_probs=True
#   - actor_rollout_ref.actor.use_rollout_log_probs=True   (yaml default)
# ============================================================================
set -xeuo pipefail

project_name='Uni-Agent-SWE-Agent'
exp_name='GRPO-Qwen3-Coder-30B-Fully-Async-decoupled-routerreplay'

RAY_DATA_HOME=${RAY_DATA_HOME:-"${HOME}/verl"}
# NOTE: Qwen3-Coder-30B-A3B-Instruct ships a native XML tool-call template, so
# no custom "-xml-template" model is needed. Make sure the agent_config tool
# parser is set to `qwen3_coder`.
...
# ============================================================================
# Trick 1: Decoupled PPO (bypass_mode=False) + Rollout Correction (Rollout IS)
# ============================================================================
...
# Optional: add rejection sampling on top of IS by appending these two flags to
# the command below (only takes effect in decoupled mode):
#   algorithm.rollout_correction.rollout_rs=seq_mean_k1 \
#   algorithm.rollout_correction.rollout_rs_threshold="0.5_2.0" \

# ============================================================================
# Trick 2: 30B MoE Router Replay (R3)
# ============================================================================
