...
PRM_EVAL_MODE=${PRM_EVAL_MODE:-false}
...
# -------- objective: Option A (default) or Option B (--distill-topk) --------
DISTILL_TOPK=${DISTILL_TOPK:-0}
DISTILL_ARGS=()
if (( DISTILL_TOPK > 0 )); then
  # Top-K reverse-KL mode.
  DISTILL_ARGS+=(
...
python3 -m uni_agent.openclaw.opd.train_entry \
  ...
  +reward.reward_model.openclaw.eval_mode=${PRM_EVAL_MODE} \
  "${DISTILL_ARGS[@]}" \
