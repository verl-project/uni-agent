...
@pytest.mark.level0
@pytest.mark.asyncio
async def test_prepared_task_snapshot_is_reused_with_runtime_endpoint(monkeypatch):
    ...
    sample = {
        ...
        "tools_kwargs": {
            "task": {
                "name": "test_task",
                "per_task_sampling": {"temperature": 0.4},
                "agent": {"name": "react", "model": {"base_url": "http://old", "temperature": 0.9}},
            }
        },
    }
    prepared, sampling = task_runner_module.prepare_task(sample)
    ...
    def unexpected_reload(*_args, **_kwargs):
        pytest.fail("prepared task must not reload task_config_path")

    monkeypatch.setattr(task_runner_module, "get_task", FakeTask)
    monkeypatch.setattr(TaskConfigResolver, "from_file", unexpected_reload)
    await run_task(
        session=SessionHandle(session_id="s1", base_url="http://gateway/sessions/s1/v1"),
        tools_kwargs=prepared["tools_kwargs"],
        raw_prompt=sample["raw_prompt"],
        task_config_path="unused.yaml",
    )
    assert captured[0]["agent"]["model"]["base_url"] == "http://gateway/sessions/s1/v1"
    assert captured[0]["per_task_sampling"] == sampling
    assert sample["tools_kwargs"]["task"]["agent"]["model"]["base_url"] == "http://old"


@pytest.mark.cpu
@pytest.mark.level0
@pytest.mark.parametrize(
    "sampling", [{"top_p": 1.5}, {"temperature": -1}, {"top_k": 2.5}, {"top_k": -2}, {"base_url": "http://bad"}]
)
def test_task_sampling_rejects_invalid_fields(sampling):
    with pytest.raises(ValueError):
        task_runner_module.prepare_task(
            {"tools_kwargs": {"task": {"name": "test_task", "per_task_sampling": sampling}}}
        )
