...
def test_run_agent_preserves_stdin_arguments_and_inherited_home(tmp_path):
    ...
    vendor_bin = tool_root / "vendor" / "x86_64-unknown-linux-musl" / "bin"
    ...
    extra_args = ["--config", "value with spaces", "quote'arg", f"$(touch {marker.name})"]
    env = os.environ.copy()
    ...
    command += " " + " ".join(shlex.quote(arg) for arg in extra_args)
    ...
    assert Path(payload["managed_package_root"]) == tool_root
...
def test_run_agent_propagates_codex_exit_code(tmp_path):
    ...
    vendor_bin.mkdir(parents=True)
    fake_codex = vendor_bin / "codex"
    fake_codex.write_text("#!/usr/bin/env python\nraise SystemExit(23)\n")
    fake_codex.chmod(0o755)
