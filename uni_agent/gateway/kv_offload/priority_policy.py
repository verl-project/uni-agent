...
def _validate_admission_config(min_offload_priority: int, admit_without_hint: bool) -> None:
    if type(min_offload_priority) is not int or not 0 <= min_offload_priority <= 100:
        raise ValueError("min_offload_priority must be an integer between 0 and 100")
    if type(admit_without_hint) is not bool:
...
class PriorityCachePolicy(CachePolicy):
    ...
    def touch(self, keys: Iterable[OffloadKey]) -> None:
        ...
        for key in reversed(list(keys)):
            ...
            if hint is None or hint.lease_until <= now:
                continue
            if meta.lease_until > now and hint.priority < meta.priority:
                # The old priority and its deadline are one claim. A lower
                # priority touch updates LRU only; it cannot renew that claim.
                continue
            if meta.lease_until > now and hint.priority == meta.priority:
                ...
                # An upgrade takes its own deadline, never the longer lease
                # of a previous lower priority. Expired claims are replaced.
...
class PriorityCPUOffloadingManager(CPUOffloadingManager):
    ...
    def __init__(
        ...
        _validate_admission_config(min_offload_priority, admit_without_hint)
        super().__init__(
        ...
        del cache_policy
        self.min_offload_priority = min_offload_priority
        self.admit_without_hint = admit_without_hint
    ...
    def prepare_store(self, keys: Collection[OffloadKey], req_context: ReqContext):
        keys_list = list(keys)
        hint, present = _parse_hint(req_context)
        if present and hint.expired():
            hint, present = _Hint(), False
...
class PriorityGPUOffloadingSpec(CPUOffloadingSpec):
    ...
    def __init__(self, vllm_config, kv_cache_config) -> None:
        ...
        min_offload_priority = extra_config.get("min_offload_priority", 0)
        admit_without_hint = extra_config.get("admit_without_hint", True)
        _validate_admission_config(min_offload_priority, admit_without_hint)
