...
class ClaudeCodeConfig(AgentConfig):
    """Black-box launch params for Claude Code (endpoint lives on :attr:`AgentConfig.model`)."""
    ...
    model_name: str | None = Field(default="claude-sonnet-4", description="Model name passed to the CLI's --model.")
...
class ClaudeCodeAgent(Agent):
    """Black-box solver: launch Claude Code in the sandbox, pointed at ``config.model``."""
    ...
    async def run(
        ...
        *,
        ...
        if cfg.model.base_url is None:
            raise ValueError("claude_code: config.model.base_url is not set (the endpoint the CLI calls)")
        ...
        argv = [
            *cfg.command,
            *(["--model", cfg.model_name] if cfg.model_name else []),
        ...
        env = {
            "ANTHROPIC_BASE_URL": cfg.model.base_url,
